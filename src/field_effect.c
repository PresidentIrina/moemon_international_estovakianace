...
static void FlightBeaconFieldEffect_FlyOutPose(struct Task *);
static void FlightBeaconFieldEffect_FlyInPose(struct Task *);
static void FlightBeaconFieldEffect_WaitBirdReturn(struct Task *);
static void FlightBeaconFieldEffect_ExclamationPoint(struct Task *);
static void FlightBeaconFieldEffect_FlyInEnd(struct Task *);
...
u8 FldEff_UseFlightBeacon(void)
{
    u8 taskId = CreateTask(Task_FlyOut, 254);
    gTasks[taskId].tMonId = gFieldEffectArguments[0];
    return 0;
...
void (*const sFlightBeaconFieldEffectFuncs[])(struct Task *) = {
    FlightBeaconFieldEffect_FlyOutPose,
    FlightBeaconFieldEffect_ExclamationPoint,
    FlyOutFieldEffect_WaitBirdLeave,
    FlyOutFieldEffect_BirdSwoopDown,
    FlyOutFieldEffect_JumpOnBird,
    FlyOutFieldEffect_FlyOffWithBird,
...
{
    if (VarGet(VAR_FLIGHT_BEACON_STATUS) == FLIGHT_BEACON_ACTIVE)
        sFlightBeaconFieldEffectFuncs[gTasks[taskId].tState](&gTasks[taskId]);
    else
...
static void FlightBeaconFieldEffect_FlyOutPose(struct Task *task)
{
    struct ObjectEvent *objectEvent = &gObjectEvents[gPlayerAvatar.objectEventId];
    if (!ObjectEventIsMovementOverridden(objectEvent) || ObjectEventClearHeldMovementIfFinished(objectEvent))
    {
...
static void FlightBeaconFieldEffect_ExclamationPoint(struct Task *task)
{
    struct ObjectEvent *objectEvent = &gObjectEvents[gPlayerAvatar.objectEventId];
    if (ObjectEventClearHeldMovementIfFinished(objectEvent))
    {
...
static void (*const sFlightBeaconFlyInFieldEffectFuncs[])(struct Task *) = {
    FlyInFieldEffect_BirdSwoopDown,
    FlyInFieldEffect_FlyInWithBird,
    FlyInFieldEffect_JumpOffBird,
    FlyInFieldEffect_FieldMovePose,
    FlightBeaconFieldEffect_WaitBirdReturn,
    FlightBeaconFieldEffect_FlyInEnd,
};

static void Task_FlyIn(u8 taskId)
{
    if (VarGet(VAR_FLIGHT_BEACON_STATUS) == FLIGHT_BEACON_ACTIVE)
        sFlightBeaconFlyInFieldEffectFuncs[gTasks[taskId].tState](&gTasks[taskId]);
    else
...
static void FlightBeaconFieldEffect_WaitBirdReturn(struct Task *task)
{
    if (GetFlyBirdAnimCompleted(task->tBirdSpriteId))
    {
        DestroySprite(&gSprites[task->tBirdSpriteId]);
...
static void FlightBeaconFieldEffect_FlyInEnd(struct Task *task)
{
    u8 state;
    struct ObjectEvent *objectEvent;
    if ((--task->data[1]) == 0)
    {
        ...
        gPlayerAvatar.flags = task->tAvatarFlags;
        gPlayerAvatar.preventStep = FALSE;
        VarSet(VAR_FLIGHT_BEACON_STATUS,FLIGHT_BEACON_OFF);
        ScriptContext_SetupScript(EventScript_FlyLanded);
        FieldEffectActiveListRemove(FLDEFF_FLY_IN);
        DestroyTask(FindTaskIdByFunc(Task_FlyIn));
    }
}
