...
static void ItemUseOnFieldCB_FlyingTool(u8);
...
void ItemUseOutOfBattle_Fly_Tool(u8 taskId)
{
    ...
    {
        sItemUseOnFieldCB = ItemUseOnFieldCB_FlyingTool;
        SetUpItemUseOnFieldCallback(taskId);
    }
}

static void ItemUseOnFieldCB_FlyingTool(u8 taskId)
{
    VarSet(VAR_FLIGHT_BEACON_STATUS,FLIGHT_BEACON_ACTIVE);
    LockPlayerFieldControls();
    ScriptContext_SetupScript(EventScript_UseFly);
    // CB2_OpenFlyMap();
    DestroyTask(taskId);
}
